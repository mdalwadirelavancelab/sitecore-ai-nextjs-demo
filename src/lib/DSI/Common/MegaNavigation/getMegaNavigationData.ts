...
// Request the standard date explicitly; do not rely on it appearing in custom fields.
...
export async function getMegaNavigationData(
  ...
  const visit = (value: unknown) => {
    ...
    if (value.componentName === 'DsiMegaNavigationComponent') renderings.push(value);
    Object.values(value).forEach(visit);
  };
  visit(result.layout.sitecore.route?.placeholders);
  if (!renderings.length) return page;
  ...
  const readChildren = (id: string): Promise<CmsItem[]> => {
    ...
    const pending = (async () => {
      ...
      do {
        ...
        if (!response.item) throw new Error(`Mega Navigation datasource item could not be read: ${id}`);
        const connection = response.item.children;
        items.push(...connection.results);
        if (!connection.pageInfo.hasNext) break;
  ...
  const readListing = (block: CmsItem): Promise<MegaLink[]> => {
    if (listingCache.has(block.id)) return listingCache.get(block.id)!;
    const pending = (async () => {
      const options = {
        headers: { ...(page.siteName ? { sc_site: page.siteName } : {}), ...headers },
        fetch: (input: RequestInfo | URL, init?: RequestInit) => fetch(input, { ...init, cache: 'no-store' }),
      };
      // Read raw reference IDs so Manual selection is not cut off by a targetItems limit.
      const response = await client.getData<{ item: {
        selected: { value: string } | null; source: { value: string } | null;
      } | null }>(`query MegaNavigationListing($id: String!, $language: String!) {
        item(path: $id, language: $language) {
  ...
  for (const rendering of renderings) {
    ...
    if (typeof datasource.id !== 'string') {
      rendering.fields = { ...fields, megaNavigation: {
    ...
    for (const menu of await readChildren(datasource.id)) {
      if (menu.template.name !== 'Dsi Mega Navigation Item') continue;
      ...
      if (['true', '1'].includes(String(enablePanel?.value).toLowerCase())) {
        for (const block of await readChildren(menu.id)) {
          if (block.template.name === 'Dsi Mega Navigation Column') {
            blocks.push({ ...toLink(block), kind: 'column', links: (await readChildren(block.id))
            ...
            blocks.push({ ...toLink(block), kind: 'listing', links: await readListing(block),
              viewAllLink: field<LinkField>(block, 'ViewAllLink'),
            });
          }
        }
      }
      items.push({ ...toLink(menu), enablePanel, blocks });
    }

...
import { Image as SitecoreImage, Link, Text, RichText, useSitecore, type ComponentParams, type LinkField, } from '@sitecore-content-sdk/nextjs';
  ...
  const content = <>    {
    ...
    <span className="mega-link-copy">
      <span className="mega-link-title">
        {item.title?.value ?
          <Text field={item.title} /> : item.link?.value?.text
        }
            ...
            {panelEnabled &&
              <div id={panelId} className="mega-panel" hidden={!expanded} aria-labelledby={triggerId}>
                <div className="mega-panel-heading">
                  ...
                  <Text tag="h2" field={item.panelTitle?.value ? item.panelTitle : item.title} />
                  ...
                  {item.blocks.map((block) =>
                    <section key={block.id} className={`mega-column mega-${block.kind}`}>
                      ...
                      {block.kind === 'column' && hasLink(block.buttonLink) &&
                        <Link field={block.buttonLink!} className="mega-column-button" />
                      }
                      ...
                      {hasLink(block.viewAllLink) &&
                        <Link field={block.viewAllLink!} className="mega-view-all" />
                      }
                    </section>)}
                </div>
